class Admin::EventInquirySettingsController < Admin::BaseController

  crudify :refinery_setting,
          :title_attribute => "name",
          :order => 'name ASC',
          :redirect_to_url => "admin_event_inquiries_url"
...
protected
  def rewrite_flash?
    ...
    flash[:notice] = flash[:notice].to_s.gsub(/(\'.*\')/) {|m| m.titleize}.gsub('EventInquiry ', '')
  end

  def save_subject_for_confirmation?
    EventInquirySetting.confirmation_subject = params[:subject] if params.keys.include?('subject')
  end
